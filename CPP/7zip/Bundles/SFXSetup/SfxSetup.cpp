    ...
    /* lpCmdLine */,int /* nCmdShow */)
{
  ...
  if (!config.IsEmpty())
  {
    ...
    UString copyProductCatalogVal = GetTextConfigValue(pairs, L"CopyProductCatalog");
    if (copyProductCatalogVal.IsEqualTo_Ascii_NoCase("yes"))
  ...
  //copy product catalog to installer cache if requested
  if (copyProductCatalog)
  {
	  TCHAR programDataFolder[MAX_PATH];
	  SHGetFolderPath(NULL, CSIDL_COMMON_APPDATA, NULL, 0, programDataFolder);

	  LPCWSTR catalogFileName = L"SolarWinds.Administration.ProductCatalog.Model.ProductCatalogModel_default.json";
	  
	  std::wstring df_str = std::wstring(programDataFolder) + L"\\SolarWinds\\Installers\\"; //intermediate step for LPCWSTR concatenation
	  std::wstring df_str_file = df_str + catalogFileName;

	  LPCWSTR destFolder = df_str.c_str();
	  LPCWSTR destFile = df_str_file.c_str();

	  if (CreateDirectory(destFolder, NULL) || GetLastError() == ERROR_ALREADY_EXISTS)
	  {
		  CopyFile(catalogFileName, destFile, false);
	  }
  }
  ...
  if (!executeFile.IsEmpty())
  {
	...
	executeParameters.Replace(L"%installerPath%", fs2us(fullPath));
	executeParameters.Replace(L"%tempPath%", fs2us(tempDir.GetPath()));
